...
function PageCreationError(message) {
    this.name = "PhantomPageCreationError";
    this.message = (message || "");
}
PageCreationError.prototype = Error.prototype;
...
PhantomInstance.prototype.initQueue = function () {
    ...
    this.queue = async.queue(function (task, taskCallback) {
        ...
        var setPageSettingsAction = function (page, callback) {
            ...
            if (!page) {
                return callback(new PageCreationError('Could not create page'));
            }
        ...
        async.waterfall(actions, function (error, result) {
            ...
            if (error) {
                if (error.name === 'HeadlessError' || error.name === 'PhantomPageCreationError') {
...
PhantomInstance.prototype.resetIdleTimer = function () {
    ...
    debug('reset idle timer to ' + this.options.idle_time + ' ms');
